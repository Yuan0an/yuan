...
echo "<h2>Addons Table</h2><pre>";
$res = $conn->query("SELECT id, name, price, type FROM addons ORDER BY id");
while ($r = $res->fetch_assoc()) echo json_encode($r) . "\n";
echo "</pre>";

echo "<h2>Payments table columns</h2><pre>";
$cols = $conn->query("SHOW COLUMNS FROM payments");
while ($c = $cols->fetch_assoc()) echo $c['Field'] . "\n";
echo "</pre>";

echo "<h2>Last 5 Bookings — addons_json raw</h2><pre>";
$res2 = $conn->query("SELECT reservation_id, addons_json FROM bookings ORDER BY id DESC LIMIT 5");
while ($r = $res2->fetch_assoc()) {
    echo "Res #" . $r['reservation_id'] . "\n";
